...
interface ConverterPresetProps {
  defaultFormat: ImageFormat;
}

export default function ConverterPreset({ defaultFormat }: ConverterPresetProps) {
  const { t } = useI18n();
  ...
  const handleConvertAll = async () => {
    ...
    for (const item of pending) {
      ...
      try {
        const blob = await convertImage(item.file, defaultFormat, (p) => updateFile(item.id, { progress: p }));
        incrementDailyCount();
        refreshRemaining();
  ...
  const handleDownload = (item: FileItem) => {
    ...
    a.href = url;
    a.download = getOutputFilename(item.file.name, defaultFormat);
    a.click();
  ...
  const handleDownloadAll = async () => {
    ...
    const zip = new JSZip();
    done.forEach((item) => { if (item.convertedBlob) zip.file(getOutputFilename(item.file.name, defaultFormat), item.convertedBlob); });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <FileList files={files} targetFormat={defaultFormat} onRemove={handleRemove} onDownload={handleDownload} />
